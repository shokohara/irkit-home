...
.controller("RoomController",
  ["$rootScope", "$scope", "$modal", "$log", "$document", "$stateParams", "$window", "$anchorScroll", "$location", "$timeout",
   "User", "Talks", "Rooms", "RoomUsers", "Entries", "Session", "Constant", "Status", "URL", "$cookies", "Blacklist"
    ($rootScope, $scope, $modal, $log, $document, $stateParams, $window, $anchorScroll, $location, $timeout, User, Talks, Rooms, RoomUsers, Entries, Session, Constant, Status, URL, $cookies, Blacklist) ->
      ...
      connect = ->
        ...
        webSocket = new WebSocket("ws://#{URL.HOST_NAME}:#{URL.PORT}/rooms/ws/#{$stateParams.id}")
        webSocket.onclose = ()->
          console.log "onclose"
          $scope.opened = false
          $scope.$apply()
